...
public class ItemSaisieResultat extends ListCell<Partie> {
	...
	private void InitialisationRadioButton(Partie item) {
		String gagnant=item.getGagnant();
		if(gagnant!=null){
			switch (gagnant) {
			case "blancGagne":
	...
	private void ChoixDuJoueurGagnant(Partie item) {
		if(item!=null){
			...
			if(noirGagne.isSelected()){
				item.joueurNoirGagne();
			}
			if(noirForfait.isSelected()){
				item.joueurNoirForfait();
			}
			if(blancGagne.isSelected()){
				item.joueurBlancGagne();
			}
			if(partieNull.isSelected()){
				item.partieNulle();
			}
			if(doubleForfait.isSelected()){
				item.doubleForfait();
			}
		}
	}
